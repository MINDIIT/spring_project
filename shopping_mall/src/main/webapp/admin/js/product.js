$(function(){
	...
	$('#product_insert_btn').click(function(){
		...
		if($('#product_name').val()==""){
			...
			if($('#product_additional_description').val()==""){
				...
				if($('#product_price').val()==""){
					...
					if($('#product_stock').val()==""){
						...
						if(!fileox){
							...
							if($('#product_detailed_description').val()==""){
								alert('상품 상세 설명을 입력하세요.');
								$('#product_detailed_description').focus();
							}else{
								$('#frm_product_insert').attr('method', 'post');
								$('#frm_product_insert').attr('action','./product_insertok.do');
								$('#frm_product_insert').attr('enctype', 'multipart/form-data');
